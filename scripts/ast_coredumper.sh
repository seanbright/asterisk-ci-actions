...
# Configuration variables can be set in the environment so we only want
# to set defaults for ones not already set.
...
# Pre-parse command line to see how we should deal with
# ast_debug_tools.conf files.
for a in "$@" ; do
	if [[ "$a" == "--no-conf-file" ]] ; then
		NO_CONF_FILE=true
	elif [[ $a =~ --conf-file=(.+)$ ]] ; then
		CONF_FILE=${BASH_REMATCH[1]}
	fi
done

if ! $NO_CONF_FILE ; then
	if [ -n "$CONF_FILE" ] ; then
		source "$CONF_FILE"
	else
...
check_gdb() {
	if [ -z "${GDB}" -o ! -x "${GDB}" ] ; then
		die -4 "${GDB} seems to not be installed. " \
		" Please install gdb or use the '--gdb' option to point to a valid executable."
	fi

	result=$($GDB --batch --ex "python print('hello')" 2>/dev/null || : )
	if [[ ! "$result" =~ ^hello$ ]] ; then
		die -4 "$GDB does not support python. " \
...
SYNOPSIS
	$prog [ --help ] [ --conf=<config file path> ] [ --no-conf-file ]
...
DESCRIPTION
	...
	--conf=<config file path>
		This is a path to an alternate ast_debug_tools.conf file from
		which to load default settings.

	--no-conf-file
		Don't load any ast_debug_tools.conf files.
